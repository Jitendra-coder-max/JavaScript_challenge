...
// let checkMail = users.every(({email}) =>  /[^@]+@[^\.]+\..+/.test(email))
...
//  let user_ip = users.filter(({ip_address}) => ip_address.split('.')[0]<128)
// let user_ip = users.filter((user) => user.ip_address.split('.')[0]<128 ).length

//  console.log(user_ip)


// Find the position in the array of the first  user object that has a Class B IP address
//    let position =       users.findIndex((user) => user.ip_address.split('.')[0]>127 &&   user.ip_address.split('.')[0]<192)
        
//    let newPostion = users.findIndex(({ip_address}) => {  const Postion=  ip_address.split('.')[0]
//    return Postion>127 && Postion<192 })

//    console.log(newPostion)

//    let Firstposition = Postion>127 && Postion<192

//  console.log(Postion>127 && Postion<192)
...
// let newUser = users.find((user) =>  user.email.includes('soundcloud'))
...
let replcaeAll = users.fill('deleted')

console.log(replcaeAll)