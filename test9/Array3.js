...
// let user_Id = users.find((user) => user.firstName === 'Madison' && user.lastName ==='Marshall')
...
//  let fname = users.find((user) => user.id === comments[0].userId)
...
//  let word = users.find((user) => user.id === comments.find((comment) => comment.text === 
//  'OK great thanks').userId)
...
// let fullName = comments.map((comment) => {
...
let nocomment = users.filter((user) => !comments.find((comment) => comment.userId === user.id))
