...
//  in it's tag property.  Get a list of all tags for 
// this account as a one-dimensional array.

// let listTag = accounts.map((account) => account.tags)

// console.log(listTag.flat(2)) 

// console.log([].concat.apply([],accounts[0].tags)
// )
// for double dimensiotn array


// let listTag=  [].concat.apply([],accounts[0].tags)

// let listTag = accounts[0].tags.flat()
...
// let checkWord = accounts[0].tags.flat().includes('consectetur')

// console.log(checkWord)

// Get a list of all the tags for all the accounts
//  and join them together as a string separated by commas.

    //   let listTag =   accounts.map((account) => account.tags.join(',')).join(',')


    //   console.log(listTag)

//     Get a list of all the friends for all the user's account
// s as an array (don't worry about duplicates)


//   */


// let allFriend = accounts.map((account) => account.friends.map((person) => person.name)).flat()

//  let allFriend  = accounts.map(({friends}) => friends.map(({name}) => name)).flat()

// console.log(allFriend)

// Get a list of all the friends for a
// ll users who are younger than 30 AND have a balance more than $1500

// let acc = accounts.map((account) => account.age < 30 && account.balance > '$1500').
// map((account) => account.friends).flat();

// console.log(acc)

// let acc = accounts
//   .filter((account) => account.age < 30 && parseInt(account.balance.replace(/\D/g,'')) > 1500)
//   .map((account) => account.friends).flat();
...
// let acc = accounts
//   .filter(({age,balance}) =>age < 30 && parseInt(balance.replaceAll(/\D/g, '')) > 1500)
//   .map(({friends}) => friends)
//   .flat();
...
//  console.log(parseInt(accounts[0].balance))
//  console.log(parseInt(accounts[0].balance.replace(/\D/g,'')))


// let acc = accounts
//   .filter((account) => account.age < 30 && parseInt(account.balance.replaceAll(/\D/g, '')) > 1500)
//   .map((account) => account.friends)
//   .flat();
...
//   .filter((account) => account.age < 30 && parseInt(account.balance.replace(/\D/g, '')) > 1500)
//   .map((account) => account.friends)
//   .reduce((friendsList, friends) => friendsList.concat(friends), []);

// console.log(acc);

let aa = accounts.filter(({balance,age}) => age <=30 && parseInt(balance.replace(/\D/g, ''))>1500)
.map(({friends}) => friends)
.flat()
